...
class ASplineCamera : AHazeCameraActor
{
	...
	UPROPERTY(DefaultComponent, Attach = FocusTracker, ShowOnActor)
    ...
    void ConstructionScript()
    {
		...
		FRotator ToFocusRot = FocusTracker.GetWorldRotation();
		bool bHasFocus = (FocusTracker.UserFocus.Weight > 0.f) || (FocusTracker.OtherPlayerFocus.Weight > 0.f) || (FocusTracker.FocusTargets.Num() > 0.f);
		if (bHasFocus)
		{
			// Turn focus tracker towards editor focus targets
			TArray<FFocusTrackerTarget> Focii;
			for (FVector EditorFocus : PreviewFocusLocations)
			{
				FFocusTrackerTarget TrackerTarget;
				TrackerTarget.Focus.Actor = this;
				TrackerTarget.Focus.LocalOffset = EditorFocus;
				TrackerTarget.Weight = 1.f;
				Focii.Add(TrackerTarget); 
			}
			FocusTracker.AllFocusTargets = Focii;
			ToFocusRot = FocusTracker.GetTargetRotation();
		}
		else
		{
			// No focus, ignore editor focus target
			FocusTracker.AllFocusTargets.Empty();
			ToFocusRot = FocusTracker.GetTargetRotation();
		}
