...
// class UMagneticLargePacketInfluencedAttachedCapability : UHazeCapability
// {
...
// 	{
// 		if(PacketOwner.GetInfluencingPlayerCount() < 2)
// 			return false;
...
// 			const FVector LeftInput = LeftPlayer.IsCody() ? GetAttributeVector(n"CodyMovementDirection") : GetAttributeVector(n"MayMovementDirection");

// 			AHazePlayerCharacter RightPlayer = Cast<AHazePlayerCharacter>(RightInfluencer.Instigator);
// 			const FVector RightInput = RightPlayer.IsCody() ? GetAttributeVector(n"CodyMovementDirection") : GetAttributeVector(n"MayMovementDirection");

// 			bool bForceMove = false;
// 			if(!RightInput.IsNearlyZero() && !LeftInput.IsNearlyZero())
// 			{
// 				bForceMove = RightInput.GetSafeNormal().DotProduct(LeftInput.GetSafeNormal()) > 0.7f;
// 			}

// 			//System::DrawDebugArrow(PacketOwner.GetActorLocation() + FVector(0.f, 0.f, 200.f), PacketOwner.GetActorLocation() + (LeftInput * 300.f) + FVector(0.f, 0.f, 200.f), LineColor = FLinearColor::Red);
// 			//System::DrawDebugArrow(PacketOwner.GetActorLocation() + FVector(0.f, 0.f, 200.f), PacketOwner.GetActorLocation() + (RightInput * 300.f) + FVector(0.f, 0.f, 200.f), LineColor = FLinearColor::Blue);

// 			FVector TargetLocation = FVector::ZeroVector;
// 			int AddedMoveInputs = 0;
// 			int AddedInputs = 0;
// 			if(bForceMove)
// 			{
// 				TargetLocation = (LeftInput + RightInput).GetSafeNormal() * PacketOwner.InAirMovespeed * 2.f;
// 				AddedInputs = 2;
// 				AddedMoveInputs = 2;
// 			}
// 			else
// 			{		
// 				if(!LeftInput.IsNearlyZero())
// 				{
// 					AddedInputs++;
// 					if(FMath::Abs(LeftInput.GetSafeNormal().DotProduct(PacketOwner.GetActorRightVector())) > 0.7f)
// 					{
// 						TargetLocation += LeftInput.GetSafeNormal() * PacketOwner.InAirMovespeed * 2.f;
// 						AddedMoveInputs++;
// 					}
// 					else
// 					{
// 						PacketOwner.SetPivotLocation(RightMagnetComponent.GetTransformFor(LeftPlayer).Location);
// 						RotateAroundPivot(LeftInput.GetSafeNormal().DotProduct(PacketOwner.GetActorForwardVector()) * PacketOwner.InAirRotationMaxAmount * DeltaTime);
// 					}
// 				}
			
// 				if(!RightInput.IsNearlyZero())
// 				{
// 					AddedInputs++;
// 					if(FMath::Abs(RightInput.GetSafeNormal().DotProduct(PacketOwner.GetActorRightVector())) > 0.7f)
// 					{
// 						TargetLocation += RightInput.GetSafeNormal() * PacketOwner.InAirMovespeed * 2.f;
// 						AddedMoveInputs++;
// 					}
// 					else
// 					{
// 						PacketOwner.SetPivotLocation(LeftMagnetComponent.GetTransformFor(RightPlayer).Location);
// 						RotateAroundPivot(RightInput.GetSafeNormal().DotProduct(-PacketOwner.GetActorForwardVector()) * PacketOwner.InAirRotationMaxAmount * DeltaTime);
// 					}
// 				}

// 				if(AddedMoveInputs > 1)
// 				{
// 					TargetLocation /= AddedInputs;
// 				}
// 			}

// 			if(AddedInputs > 1)
// 			{
// 				FVector DeltaMovement = FMath::VInterpConstantTo(PacketOwner.GetActorLocation(), PacketOwner.GetActorLocation() + TargetLocation, DeltaTime, PacketOwner.InAirMovespeed * AddedMoveInputs * 0.5f);
