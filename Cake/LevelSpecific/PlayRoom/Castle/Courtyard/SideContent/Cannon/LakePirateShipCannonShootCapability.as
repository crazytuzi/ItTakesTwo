...
class ULakePirateShipCannonShootCapability : UCharacterMovementCapability
{	
	...
	void PreTick(float DeltaTime)
	{
		// if (!IsActive())
		// {
		// 	if(CannonComponent.CannonActor != nullptr)
		// 	{
		// 		UHazeCrumbComponent::Get(CannonComponent.CannonActor.InteractingPlayer).LeaveAndTriggerDelegateCrumb(FHazeCrumbDelegate(CannonComponent.CannonActor, n"Crumb_ShootCannon"), FHazeDelegateCrumbParams());
		// 	}
		// }
	}
	...
	void OnActivated(FCapabilityActivationParams ActivationParams)
	{
		...
		FVector StartVel = CannonComponent.CannonActor.ShootDirection.ForwardVector * 10000.f;
	...
	{	
		if (HasControl())
		{
			...
			Velocity -= Velocity * 0.125f * DeltaTime;
			Velocity = RotateVectorTowardsAroundAxis(Velocity, MoveDirection, MoveComp.WorldUp, 22.f * DeltaTime);
