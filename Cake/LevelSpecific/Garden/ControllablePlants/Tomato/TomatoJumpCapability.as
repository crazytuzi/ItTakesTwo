...
class UTomatoJumpCapability : UHazeCapability
{
	...
	{
		if (IsActioning(TomatoTags::Jump) && Tomato.bTomatoInitialized && !Tomato.bIsJumping)
		{
	...
	{
		if (!Tomato.bTomatoInitialized || (Tomato.bIsJumping && !MoveComp.IsAirborne()))
		{
	...
	{		
		Tomato.bIsJumping = true;
		const float Impulse = CalculateVerticalJumpForceWithInheritedVelocityAndApplyHorizontalVelocityAsImpulse(GetJumpImpulse());
	...
	{
		return Settings.JumpHeight / Tomato.GetTomatoScale();
	}
}
