...
class UDiscoBallJumpCapability : UCharacterMovementCapability
{
	...
	{	
		...
		StartJumpWithInheritedVelocity(JumpData, MoveComp.JumpSettings.FloorJumpImpulse);
		...
		if (!AllowCameraVerticalMovement(CurrentHeight))
			Player.ApplyPivotLagSpeed(FVector(0.5f, 0.5f, 0.f), this, EHazeCameraPriority::Script);
		Player.ApplyPivotLagMax(FVector(200.f, 200.f, 400.f), this, EHazeCameraPriority::Script);
	...
	void MakeFrameMovementData(FHazeFrameMovement& FrameMoveData, float DeltaTime)
	{
		...
		{	
			...
			// Have added divided by two for less air control
			...
			//This is tweaked from original jumpcapability
			FrameMoveData.ApplyDelta(MoveInput * MoveSpeed * DeltaTime);
